...
- **Phase 1 (Desktop Testing):** ⚠️ **BLOCKED — the 750W PSU this plan depends on has blown.** The Corsair HX750i is dead; the power budget below was sized against that specific unit and must be re-derived against whatever replaced it. Confirm first whether the backup PSU was consumed to revive the desktop, since that would also leave the second build short a PSU. See `machines/parts/README.md`. ~~Test dual 3090s (existing 3090 + EVGA XC3) on current desktop (MSI Z390 / i9-9900KF) using existing 750W PSU with power capping (`nvidia-smi -pl 200` or `250` for ~500W–550W total system load).~~ Validate 48GB VRAM pool, vLLM / Camelid / Ollama multi-GPU tensor parallelism.
